...
DROP TABLE TB_WISHI CASCADE CONSTRAINTS;
...
DROP SEQUENCE SEQ_REVIEW_ATT_NO;
DROP SEQUENCE SEQ_DELI_CODE;
DROP SEQUENCE SEQ_REV_ATT_NO;
DROP SEQUENCE SEQ_SIRIAL_NO;
...
COMMENT ON COLUMN TB_PRODUCT.PROD_NO IS '상품 번호';
COMMENT ON COLUMN TB_PRODUCT.CATEGORY_NAME IS '카테고리 이름';
COMMENT ON COLUMN TB_PRODUCT.SUBCATEGORY_NAME IS '서브카테고리 이름';
COMMENT ON COLUMN TB_PRODUCT.PROD_NAME IS '상품 이름';
COMMENT ON COLUMN TB_PRODUCT.PROD_CONTENT IS '상품 설명';
COMMENT ON COLUMN TB_PRODUCT.PROD_PRICE IS '상품 가격';
COMMENT ON COLUMN TB_PRODUCT.PROD_ORIGIN IS '상품 원산지';
COMMENT ON COLUMN TB_PRODUCT.PROD_CAUTION IS '주의 사항';
COMMENT ON COLUMN TB_PRODUCT.PROD_COUNT IS '상품 재고 수량';
COMMENT ON COLUMN TB_PRODUCT.PROD_ENROLL_DATE IS '상품 등록일';
COMMENT ON COLUMN TB_PRODUCT.PROD_STATUS IS '상품 상태 (판매/미판매)';
...
COMMENT ON COLUMN TB_INQUIRY.INQUIRY_TITLE IS '문의 제목';
...
CREATE TABLE TB_OPTION (
    ...
    OPT_VALUE VARCHAR2(90) NOT NULL,
...
CREATE TABLE TB_REVIEW (
    ...
    CONSTRAINT FK_ORDER_TO_REVIEW FOREIGN KEY (ORDER_NO) REFERENCES TB_ORDER (ORDER_NO),
...
COMMENT ON COLUMN TB_REVIEW.REV_TITLE IS '리뷰 제목';
...
COMMENT ON COLUMN TB_REVIEW.ORDER_NO IS '주문 번호';
COMMENT ON COLUMN TB_REVIEW.MEMBER_ID IS '회원 아이디';

-- TB_WISHI 테이블 생성
CREATE TABLE TB_WISHI (
    MEMBER_ID VARCHAR2(20) NOT NULL,
    PROD_NO NUMBER NOT NULL,
    CONSTRAINT PK_WISHI PRIMARY KEY (MEMBER_ID, PROD_NO),
    CONSTRAINT FK_MEMBER_TO_WISHI FOREIGN KEY (MEMBER_ID) REFERENCES TB_MEMBER (MEMBER_ID),
    CONSTRAINT FK_PRODUCT_TO_WISHI FOREIGN KEY (PROD_NO) REFERENCES TB_PRODUCT (PROD_NO)
);

COMMENT ON COLUMN TB_WISHI.MEMBER_ID IS '회원 아이디';
COMMENT ON COLUMN TB_WISHI.PROD_NO IS '상품 번호';
